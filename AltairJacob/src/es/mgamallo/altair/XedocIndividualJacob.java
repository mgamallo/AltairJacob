...
public class XedocIndividualJacob {
	...
	public XedocIndividualJacob(String nombreFichero, ActiveXComponent navegador){
...
//////////////////////////////////////////////////////	
		...
		for(int i=0;i<InicioAltairJacob.listaPdfs.length;i++){
			if(InicioAltairJacob.listaPdfs[i].equals(nombreFicheroCorregido)){
				System.out.println("El fichero ha sido encontrado y es el número " + (i+1));
				numeroDelPdf = i+1;
				break;
			}
		}
	...
	public void seleccionarServicio(){
		...
		if(!servicio.equals(SERVICIO_DESCONOCIDO)){
			...
			/*
			Select seleccion = new Select(driver.findElement(By.id("{hc}servicioEspecialidad-{hc}docExt")));
			seleccion.selectByIndex(5);
			*/
		...
		public void seleccionarDocumento(){
			...
			//			System.out.println(nombreCompletoServicioOpcion);
				...
				cadena = ""
						+ ""
						+ "var selectTipoDocumento = document.getElementById('{hc}codDocEx-{hc}docExt');"
						+ "var opciones = selectTipoDocumento.options;"
						+ "var numOpciones = opciones.length;"
						+ "for(var i = 0;i<numOpciones;i++){"
							+ "if(opciones[i].text.localeCompare('" + tipoDocumento + "') == 0){"
									+ "opciones[i].selected = 'true';"
									+ "document.getElementById('cajaColoreada0').value = '" + tipoDocumento + "';"
									+ "break;"
							+ "}"
						+ "}"
					//	+ "alert('fin');"
		...
		public void inicializaNodosHospUrgQui (){
			...
			Dispatch.put(IE, "Visible", true);
		      ...
		      while(true){
		    	  ...
		    	  Variant ca = Dispatch.call(IE, "readyState");
		    	  System.out.println(ca.toString());
		    	  if(Integer.valueOf(ca.toString()) ==4){
			...
			Dispatch theDocument = Dispatch.call(IE, "Document").toDispatch();
			...
			*/
			...
			Dispatch window = Dispatch.call(documento, "parentwindow").toDispatch();
			Dispatch.call(window, "executeScript",cadena);
		     */  
		...
		//	Dispatch.call(navegador, "navigate","javascript:"+ cad01);
...
var hola = document.getElementById('tablameritos');
undefined
hola.innerHTML;
No se puede obtener la propiedad 'innerHTML' de referencia nula o sin definir
var hola = document.getElementById('tablaMeritos');
undefined
hola.setAttribute('onclick','alert(this.id)');
undefined
var ides = hola.querySelectorAll('nodoSeleccionado');
undefined
hola.length;
undefined
ides.length;
...
var ides = hola.querySelectorAll('.');
SyntaxError
var ides = hola.querySelectorAll('#nodoSeleccionado');
undefined
ides.length;
...
undefined
ides[0].innerHTML;
"<ins class="jstree-checkbox">&nbsp;</ins><ins class="jstree-icon">&nbsp;</ins>
    				HOS-DIGM-24/01/2016-02/02/2016
			"
ides[0].setAttribute('hola','adios');
undefined
ides[0].setAttribute("onclick","alert('hola')");
undefined
ides[0].setAttribute("onclick","if(this.id.localeCompare('nodoSeleccionado') == 0){this.id='';}");
undefined

ides[0].setAttribute("onclick","if(this.id.localeCompare('nodoSeleccionado') == 0){this.id='';}else{this.id='nodoSeleccionado';}");
var padre = ides[0].parentNode;
ides[0].setAttribute("onclick","if(this.id.localeCompare('nodoSeleccionado') == 0){this.id='';}else{this.id='nodoSeleccionado';var padre = this.parentNode;padre.class='jstree-last jstree-leaf jstree-unchecked';}");

ejem2.setAttribute("onclick","if(this.getAttribute('class').localeCompare('jstree-last jstree-leaf jstree-unchecked') == 0){ alert('sin ok');var elegidos = document.querySelectorAll('#nodoSeleccionado');elegidos.id='';}");
